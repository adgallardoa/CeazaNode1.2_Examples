...
int W25N::begin(uint32_t cs){
  ...
  //Serial.print("MAN_ID:");Serial.println(jedec[2],HEX); Serial.print("DEV_ID:");Serial.print(jedec[3],HEX);Serial.println(jedec[4],HEX);
  //Serial.println((uint16_t)jedec[3]<< 8,HEX);
  //Serial.println((uint16_t)jedec[4],HEX);
  //Serial.println((uint16_t)((uint16_t)jedec[3] << 8 | (uint8_t)jedec[4]),HEX);
  if((uint8_t)jedec[2] == WINBOND_MAN_ID){
    if((uint16_t)(jedec[3] << 8 | jedec[4]) == W25N01GV_DEV_ID){
      this->setStatusReg(W25N_PROT_REG, 0x00);
      this->_model = W25N01GV;
	  //Serial.println("MAN_ID OK");
      return 0;
    }
...
int W25N::loadProgData(uint16_t columnAdd, char* buf, uint32_t dataLen){
  ...
  SPI.transfer((uint8_t*)buf, dataLen);
  digitalWrite(_cs, HIGH);
  SPI.endTransaction();
  return 0;
}
